...
-- or
Select a.method from payment a inner join
(select faremethod,sum(fare)fare from trips 
Group by faremethod
...
select loc_to,count(loc_to) from trips
group by loc_to
...
select driverid,sum(fare) sum
from trips
group by driverid 
order by sum(fare) desc
limit 5;

-- or (there can be multiple drivers earning same amount so to find top 5 drivers use dense rank function)
...
select duration,count(duration) as no_of_trips from trips
group by duration
